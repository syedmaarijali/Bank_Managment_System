...
public class SignupOne extends JFrame implements ActionListener{
    ...
    JDateChooser dchooser;
        ...
        SignupOne(){
            ...
            dchooser.setBounds(300,240,400,30);
            dchooser.setForeground(Color.BLACK);
            add(dchooser);
            ...
            male = new JRadioButton("Male"); // Create Radio buttons
            ...
            ButtonGroup gendergroup = new ButtonGroup();// Group buttons together so only one can be selected at a time
            ...
            ButtonGroup maritalgroup = new ButtonGroup();// Group buttons together so only one can be selected at a time
        ...
        public void actionPerformed(ActionEvent ae){
            ...
            String dob = ((JTextField)dchooser.getDateEditor().getUiComponent()).getText();
            String gender = null;
            if(male.isSelected()){
                gender = "Male";
            ...
            try{
                if(name.equals("") || fname.equals("") || email.equals("") || address.equals("") ||
                   city.equals("") || state.equals("") || pin.equals("") || gender == null || marital == null){
                   JOptionPane.showMessageDialog(null, "All fields are required!");
                   return;
                } else {
                    
                  Conn conn = new Conn();
                  String query = "insert into signup(formno, name, father_name, dob, gender, email, marital, address, pin, city, state) values (?,?,?,?,?,?,?,?,?,?,?)";
                  PreparedStatement p = conn.c.prepareStatement(query);
                  
                    p.setString(1, formno);
                    p.setString(2, name);
                    p.setString(3, fname);
                    p.setString(4, dob);
                    p.setString(5, gender);
                    p.setString(6, email);
                    p.setString(7, marital);
